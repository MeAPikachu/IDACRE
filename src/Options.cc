...
Options::Options(std::shared_ptr<MongoLog>& log, std::string options_name, std::string hostname,
  ...
  {
   std::cout << "Initializing options class" << std::endl ; 
  }
  ...
  // fLog->Entry(MongoLog::Local, "baseline_dac_mode %s",(std::string)GetString("baseline_dac_mode")  ) ;
  ...
  fLog->Entry(MongoLog::Local, "We generated a bool load_ref %d", (uint32_t)load_ref) ;
...
int Options::Load(std::string name, mongocxx::collection* opts_collection, std::string override_opts) 
{
  ...
  // std::cout <<  "Mongo mode:" << name << std::endl ; 
